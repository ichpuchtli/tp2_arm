...
void DACSPI_Write(unsigned long usData);
...
int main(void) {
    ...
    while(1)
    {
        DACSPI_Write(sinetable[i++ & 255] & 4095);
...
void DACSPI_Init(void){
    ...
    SSIConfigSetExpClk(SSI0_BASE, SysCtlClockGet(), SSI_FRF_MOTO_MODE_0, SSI_MODE_MASTER, SysCtlClockGet()/2, 16);
